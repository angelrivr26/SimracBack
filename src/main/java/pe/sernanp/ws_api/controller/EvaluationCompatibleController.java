...
@RestController
@RequestMapping("/evaluationcompatible")
public class EvaluationCompatibleController extends BaseController {

    @Autowired
    EvaluationCompatibleService _service;

    @PostMapping()
    @ApiOperation(value = "Método que permite registrar la evaluacion compatible", notes = "")
    public ResponseEntity<EvaluationCompatible> save(@RequestParam("item") String item, @RequestParam("file") MultipartFile file) throws Exception {
        ResponseEntity<EvaluationCompatible> response = new ResponseEntity<>();
        EvaluationCompatible item2 = super.fromJson(item, EvaluationCompatible.class);
        response = this._service.save(item2, file);
        return response;
    }

    @PutMapping()
    @ApiOperation(value = "Método que permite actualizar la evaluacion compatible", notes = "")
    public ResponseEntity<EvaluationCompatible> update(@RequestParam("item") String item, @RequestParam("file") MultipartFile file) throws Exception {
        ResponseEntity<EvaluationCompatible> response = new ResponseEntity<>();
        EvaluationCompatible item2 = super.fromJson(item, EvaluationCompatible.class);
    ...
    @ApiOperation(value = "Método que devuelve listado de evaluacion compatible por idTramite", notes = "")
    public ResponseEntity<EvaluationCompatible> listByTramite(@PathVariable("id") int id) throws Exception {
        ResponseEntity<EvaluationCompatible> response = new ResponseEntity<>();
    ...
    @ApiOperation(value = "Método que el devuelve documento de evaluacion compatible", notes = "")
    @ResponseBody()
